...
function todaysEntries(entries) {
  ...
  //   The function should return the names of those entries whose date is the current date
  const todayEntries = entries.filter((entry) =>
    entry.date.startsWith(currentDate)
  );
  //   here's an array of three entries
...
const result = todaysEntries(entries);
console.log(result);
